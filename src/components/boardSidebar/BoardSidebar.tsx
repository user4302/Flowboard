...
import { useBoardSidebarState } from './hooks';

/**
 * BoardSidebar component - Main navigation sidebar for the Flowboard application
...
export function BoardSidebar() {
  ...
  const { isCreatingBoard, setIsCreatingBoard } = useBoardSidebarState();
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed left-0 top-0 z-50 h-screen w-64 transform border-r border-slate-200 bg-white transition-transform duration-300 dark:border-slate-700 dark:bg-slate-900 md:translate-x-0',
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        )}
      >
        <div className="flex h-full flex-col">
          {/* Header - App branding and mobile close button */}
          <BoardSidebarHeader onClose={() => setSidebarOpen(false)} />
